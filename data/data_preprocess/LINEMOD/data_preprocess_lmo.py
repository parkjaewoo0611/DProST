...
class Preprocessor():
    def __init__(self, mode, name=None):
        ...
        self.mode = mode
        if mode == 'test':
            self.DATA_PATH = 'data/OCCLUSION/test'
        else:
            self.DATA_PATH = 'data/OCCLUSION/train'
        self.BBOX_PATH = 'data/OCCLUSION/test_bboxes'
        self.INDEX_PATH = 'data/OCCLUSION/index'
    ...
    def build(self):
        ...
        if self.mode == 'test':
            self.bbox_faster = self.load_test_bbox()
        return self
        ...
        for scene_id in scene_list:
            ...
            for im_id in scene_gt.keys():
                ...
                for inst_gt, inst_in in zip(scene_gt[im_id], scene_in[im_id]):
                    RT = R_T_to_RT(np.array(inst_gt['cam_R_m2c']).reshape(3, 3),
    ...
    def load_test_bbox(self):
        with open(os.path.join(self.BBOX_PATH, 'faster_R50_FPN_AugCosyAAE_HalfAnchor_lmo_pbr_lmo_fuse_real_all_8e_test_480x640.json')) as f:
            faster = json.load(f)
        ...
        for key in keys:
            ...
            for obj_id in self.obj_list:
                obj_infos = [info for info in faster[key] if info['obj_id'] == obj_id]
                if obj_infos == []:
                    f_bbox = [270, 190, 370, 290]         ## --> any useless bbox to prevent in-out error
                else:
                    obj_idx = np.argmax(np.array([info['score'] for info in obj_infos]))
                    obj_info = obj_infos[obj_idx]
                    f_bbox = obj_info['bbox_est']
                    f_bbox[2] += f_bbox[0]
                    f_bbox[3] += f_bbox[1]
    ...
    def load(self, scene_id, im_id, obj_id):
        ...
        RT = self.gt_info[scene_id][im_id][obj_id]['RT']
        ...
        if self.mode == 'test':
            bbox_faster = self.bbox_faster[scene_id][im_id][obj_id]['bbox_faster']
